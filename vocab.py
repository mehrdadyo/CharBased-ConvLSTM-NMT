...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sent_char_indices_pad = pad_sents_char(sent_char_indices, self.char2id['∏'])
        sents_var = torch.tensor(sent_char_indices_pad, dtype=torch.long, device=device)
        sents_var = sents_var.contiguous().permute(1, 0, 2).contiguous()
        return sents_var
